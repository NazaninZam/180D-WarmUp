...
while(1):
    ...
    # for red 
    lower_red = np.array([136, 87, 111], np.uint8)  # define range of blue color in HSV
    ...
    blue_contours, hierarchy = cv.findContours(blue_mask, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)
    
    red_contours, hierarchy = cv.findContours(red_mask, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)
